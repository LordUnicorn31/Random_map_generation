...
// TODO 1: Create an enum class called "Tile"
enum class Tile
{
	Unused,
	DirtWall,
...
// TODO 2: Create an enum class called "Direction"
...
public:
	...
	{
		// TODO 3: FILL THIS FUNCTION TO CHECK THE X LIMIT OF THE GAME
		return x >= 0 && x < xSize;
	}

	bool IsYInBounds(int y) const
	{
		// TODO 3: FILL THIS FUNCTION TO CHECK THE Y LIMIT OF THE GAME
		return y >= 0 && y < ySize;
	}
...
private:
	...
	{
		// TODO 4:Fill this function that generates random numbers
	...
	bool MakeRoom(Map& map, RngT& rng, int x, int y, int xMaxLength, int yMaxLength, Direction direction) const
	{
		...
		// TODO 5: set the xLength and the yLength of the room, using the GetRandomInt function is necessary
		...
		if (!map.IsAreaUnused(xStart, yStart, xEnd, yEnd))
			return false;
		//TODO 6: Create the room with the function SetCells (from the class Map)
	...
	bool MakeFeature(Map& map, RngT& rng, int x, int y, int xmod, int ymod, Direction direction) const
	{
		...
		// TODO 7: make a variable called "Chance" that gets a random number between 0 and 100
	...
	{
		// TODO 8: Make one room in the middle to start things off.
